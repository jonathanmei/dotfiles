---
name: debate
description: Two coding agents debate the merits of a piece of text through a shared DEBATE.md file. Use when you want adversarial review of a commit, diff, PR, or implementation plan.
argument-hint: "[A|B] [subject: commit, diff, PR, or file to debate]"
---

# Debate Protocol

A protocol for two coding agents (Claude Code, Gemini CLI, Codex, or any other) to debate a document through a shared file (`DEBATE.md`). Both agents receive this same prompt. Role is determined by the first argument.

## How to use

Open two terminal tabs with coding agents (same or different). Invoke this skill in each, specifying role and subject:

- `/debate A <subject>` — opener, creates `DEBATE.md` with the first analysis
- `/debate B <subject>` — responder, waits for `DEBATE.md` then replies
...
- Be specific. Reference file paths, line numbers, code snippets, URLs, mathematical expressions.
...
- `DEBATE.md` is a scratch file; delete it after the debate. You may commit non-obvious findings to `MEMORY.md`.
